...
@dataclass
class Conversation:
    """Clean conversation state management with chat history tracking"""
    ...
    def get_context_summary(self) -> str:
        ...
        if self.clarifications_asked:
            parts.append(f"Clarifications Asked: {len(self.clarifications_asked)}/{MAX_CLARIFICATIONS}")
...
class SkippyCoach:
    ...
    def _create_unified_prompt(self) -> ChatPromptTemplate:
        """Create unified prompt template with conversation history"""
...
Remember to build upon previous responses and maintain conversation continuity:
""")
    ...
    def _generate_response(self, question: str, docs: List[Document], 
        ...
        """Generate coaching response with conversation history"""
        if not docs:
...
class UIComponents:
    ...
    def show_memory_indicator(conversation: Conversation):
        ...
        if conversation.original_question:
            ...
            history_info = (
                f" | 💬 {len(conversation.chat_history)} messages"
                if conversation.chat_history else ""
            )
            st.markdown(f"""
            <div class="memory-badge">
            🧠 <strong>Coaching Context:</strong> {conversation.original_question[:60]}...{clarification_info}{history_info}
            </div>
            """, unsafe_allow_html=True)
    ...
    def display_message(message: Dict[str, str]):
        ...
        if message["role"] == "user":
            ...
            st.markdown(
                f'<div class="coach-message">'
                f'<div><strong>🚀 Coach Skippy:</strong><br>{message["content"]}</div>'
                f'<div class="message-time">{timestamp}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
    ...
    @staticmethod
    def display_chat_container(messages: List[Dict[str, str]]):
        ...
        with st.sidebar:
            ...
            **Memory Intelligence:**
            ...
            if conversation.original_question:
                ...
                **Messages:** {len(conversation.chat_history)}
                """)
...
def main():
    ...
    with col1:
        ...
        # Display chat container
        UIComponents.display_chat_container(messages)
        ...
        with st.form("coach_form", clear_on_submit=True):
            ...
            with col_input:
                user_input = st.text_input(
                    "Type your SAP EWM question here...",
        ...
        if submitted and user_input:
            ...
            with st.spinner("🧠 Coaching with enterprise intelligence..."):
                ...
                response = coach.ask(user_input, conversation, st.session_state.messages)
